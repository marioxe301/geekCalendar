...
void put_days(std::vector<std::string> &format, int start, int end, int day,
  ...
  int dayc = 1;
  int initial_index =
      (start == 0) ? 0 : (start * size_of_row); // a este se le va sumar el
                                                // size_of_row en cada iteracion
  // si el tamaño es mayor a 39 se reinicia a 0 el initial index y se le suma al
  // index + 1
  for (dayc; dayc <= end; dayc++) {
    if (initial_index < 39) {
      if (dayc < 10) {
        format[index].replace(initial_index, 3,
                              " " + std::to_string(dayc) + " ");
      } else {
